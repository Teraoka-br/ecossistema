...
function seedHisImport(db: Db): number {
  ...
  const r = db.prepare(
    `INSERT INTO his_imports (filename, file_hash, status, rows_found, issues_count, created_at, finished_at)
     VALUES ('his.xlsx', ?, 'COMPLETED', 1, 0, datetime('now'), datetime('now'))`,
  ).run(hash);
  return r.lastInsertRowid as number;
}

function seedHisCurrent(db: Db, importId: number, imei: string, ageDays: number | null, cost: number | null) {
  db.prepare(
    `INSERT OR REPLACE INTO his_current (his_import_id, imei_norm, age_days, audited_cost, row_hash)
     VALUES (?, ?, ?, ?, 'seed')`,
  ).run(importId, imei.replace(/\D/g, ""), ageDays, cost);
}
...
function seedSerialCurrent(db: Db, importId: number, imei: string, model: string, codComercial: string, deposito: string, disponivel: string, fabricante = "Samsung") {
  db.prepare(
    `INSERT OR REPLACE INTO rel_seriais_current
       (rel_seriais_import_id, imei_norm, serial, descricao, codigo_comercial, fabricante, disponivel, deposito_atual, row_hash)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'seed')`,
  ).run(importId, imei.replace(/\D/g, ""), imei, model, codComercial, fabricante, disponivel, deposito);
}
...
function seedShOsCurrent(db: Db, importId: number, opts: {
  ...
  const lookupKey = opts.osNorm || `I:${opts.imeiNorm ?? ""}`;
  db.prepare(
    `INSERT OR REPLACE INTO sh_os_current
       (sh_os_import_id, lookup_key, os_norm, imei_norm, os_raw, imei_raw, marca, modelo, cor, defeito, obs_servico, row_hash)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'seed')`,
  ).run(
    importId, lookupKey, opts.osNorm, opts.imeiNorm ?? null,
...
describe("getPrefill — 15 cenários", () => {
  ...
  it("4. SH por OS → mesmos campos preenchidos", () => {
    ...
    seedShOsCurrent(db, shId, {
      ...
      imeiNorm: undefined, imeiRaw: undefined,
  ...
  it("5. Rel. Seriais por IMEI → codigoComercial + deposito", () => {
    ...
    seedSerialCurrent(db, serId, "351234567890123", "Galaxy A32", "COD-001", "DP-CURITIBA", "SIM");
  ...
  // 6. His: dados corretos retornados do his_current
  it("6. His dados de his_current → custo e idade corretos", () => {
    const hisId = seedHisImport(db);
    seedHisCurrent(db, hisId, "351234567890123", 45, 750);
  ...
  it("7. Custo NULL no His → custo=0 + warning HIS_NO_COST", () => {
    const hisId = seedHisImport(db);
    seedHisCurrent(db, hisId, "351234567890123", 20, null);
  ...
  it("8. PEACS exact match → vendaEstimada preenchida", () => {
    const serId = seedRelSeriaisImport(db);
    seedSerialCurrent(db, serId, "351234567890123", "Galaxy A32", "SAMSUNG GALAXY A32", "DP-1", "SIM");
  ...
  it("9. PEACS não encontrado → vendaEstimada=0 + warning PEACS_NOT_FOUND", () => {
    const serId = seedRelSeriaisImport(db);
    seedSerialCurrent(db, serId, "351234567890123", "Galaxy A32", "COD-INEXISTENTE", "DP-1", "SIM");
  ...
  // 10. Rel. Seriais disponivel=NÃO → dados retornados mesmo assim
  it("10. Rel. Seriais disponivel=NÃO → dados retornados corretamente", () => {
    const serId = seedRelSeriaisImport(db);
    seedSerialCurrent(db, serId, "351234567890123", "Galaxy A32", "COD-X", "DP-CURITIBA", "NÃO");

    const r = getPrefill(db, "351234567890123");
    expect(r.codigoComercial).toBe("COD-X");
    expect(r.deposito).toBe("DP-CURITIBA");
    expect(r.sources["codigoComercial"]).toBe("SERIAIS");
  });
  ...
  it("11. idade=0 no His → retorna 0, não nulo", () => {
    const hisId = seedHisImport(db);
    seedHisCurrent(db, hisId, "351234567890123", 0, 800);
  ...
  it("14. SH sem modelo, Seriais com modelo → usa Seriais", () => {
    ...
    const serId = seedRelSeriaisImport(db);
    seedSerialCurrent(db, serId, "351234567890123", "iPhone 11", "COD-X", "DP-1", "SIM");
  ...
  // 17. rel_seriais_current usa descricao como modelo (não produto)
  it("17. rel_seriais_current.descricao é o modelo (produto ignorado)", () => {
    const serId = seedRelSeriaisImport(db);
    db.prepare(
      `INSERT INTO rel_seriais_current
         (rel_seriais_import_id, imei_norm, serial, descricao, codigo_comercial, fabricante, disponivel, deposito_atual, row_hash)
       VALUES (?, ?, ?, ?, ?, ?, 'SIM', 'DP-1', 'seed')`,
    ).run(serId, "351234567890123", "351234567890123", "iPhone 12 64GB", "APPL-IP12-64", "Apple");
  ...
  it("18. Fallback Seriais: modelo vem de descricao em rel_seriais_current", () => {
    const serId = seedRelSeriaisImport(db);
    db.prepare(
      `INSERT INTO rel_seriais_current
         (rel_seriais_import_id, imei_norm, serial, descricao, codigo_comercial, fabricante, disponivel, deposito_atual, row_hash)
       VALUES (?, ?, ?, ?, ?, ?, 'SIM', 'DP-X', 'seed')`,
    ).run(serId, "999000000000001", "999000000000001", "Galaxy S22 128GB", "SAMS-S22-128", "Samsung");
  ...
  it("19. Idade preservada exatamente do His — sem recálculo", () => {
    const hisId = seedHisImport(db);
    seedHisCurrent(db, hisId, "351234567890123", 183, 650);

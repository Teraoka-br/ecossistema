...
export function createRepairCase(
  ...
  // Bloquear se já existe caso ATIVO com o mesmo IMEI (exceto na migração)
  if (imeiNorm && !params.legacyDeviceKey) {
    const existing = db
      .prepare(
        `SELECT id FROM repair_cases WHERE imei_norm = ? AND workflow_status NOT IN ('CONCLUIDO','VENDA_ESTADO','CANCELADO') LIMIT 1`,
      )
      .get(imeiNorm) as { id: number } | undefined;
    if (existing) {
      throw new RepairError("DUPLICATE_ACTIVE_IMEI", `Já existe um caso ativo para o IMEI ${params.imei} (id ${existing.id}).`);
    }
  }
  ...
  const res = db
    .prepare(
      `INSERT INTO repair_cases
         (imei, imei_norm, os, os_norm, brand, model, entry_date, age_days,
          ...
          assigned_technician_id, legacy_import_batch_id, legacy_device_key,
    ...
    .run(
      ...
      params.entryDate ?? null, params.ageDays ?? null,
...
interface RepairCaseRow {
  ...
  brand: string | null; model: string | null; entry_date: string | null; age_days: number | null;
  cost: number | null; estimated_sale: number | null; margin: number | null; notes: string | null;
  analysis_status: string; workflow_status: string; assigned_technician_id: number | null;
  manual_priority_active: number; legacy_import_batch_id: number | null; legacy_device_key: string | null;
...
function toRepairCase(r: RepairCaseRow): RepairCase {
  return {
    ...
    brand: r.brand, model: r.model, entryDate: r.entry_date, ageDays: r.age_days,
    cost: r.cost, estimatedSale: r.estimated_sale, margin: r.margin, notes: r.notes,
    ...
    legacyImportBatchId: r.legacy_import_batch_id, legacyDeviceKey: r.legacy_device_key,

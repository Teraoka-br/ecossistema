...
1. **Camada canônica de custos de peças + margem de reparo em modo sombra** (2026-07-22, migrations 058–061, branch `claude/canonical-parts-cost-layer-073927`): (a) `part_price_events` — histórico imutável de preços (correção = novo evento), alimentado automaticamente por aprovação de cotação (`cotacao-service`) e recebimento (`receiving-service`), com backfill idempotente (`part-price-backfill.ts`). (b) Resolução canônica de custo por contexto (`cost-resolution-service.ts`: CURRENT_REPAIR / PURCHASE_SIMULATION / HISTORICAL_EVALUATION, confiança HIGH→CONFLICT, staleness, fallback por compatibilidade com rebaixamento) + `calculateRepairPartsCost` (cobertura, fingerprint) + `calculateRepairMargin` pura. (c) Motor: regra ganhou `include_parts_cost`/`shadow_mode`/`min_parts_cost_coverage`/`missing_cost_behavior`; em sombra calcula margem/score de reparo em paralelo SEM alterar a fila (validado: fila idêntica ao legado na cópia do beta); resultados persistidos em `repair_match_case_results`. (d) Projeção de cotação ganhou `incrementalPartsCost`/`incrementalRepairMargin` (preço da seleção para chaves cotadas) e histórico por linha. (e) Venda no Estado: política configurável na regra ativa (`as_is_*`, padrão 50% / 10 candidatos / aprovação humana), classificação em `case_economic_evaluations` separada do workflow — só `approveAsIs` move para VENDA_ESTADO; decisões humanas preservadas. (f) Override manual (`part_cost_overrides`, permissão `MANAGE_PART_COSTS`): precedência na resolução, imune a importação, restauração auditada. (g) UI: campos de custo nas Regras do Match; modal de histórico de preços + override em `/estoque/referencias`. Validação na cópia do beta (`scripts/validate-parts-cost.ts`, `data/app-beta-copy.sqlite`): 426 eventos backfill, 142 chaves com preço, 707 casos abertos (167 cobertura total, 225 margens mudariam), sombra idêntica ao legado, score real com peças mudaria 2 status e 625 posições (NÃO ativado — exige aprovação explícita). 711 testes verdes.
2. **Correção de regressões deixadas pela sessão de UI/VENDA_ESTADO do dia 2026-07-20** (sem migration nova): os commits da manhã (`78f5e00`, `84269d4`, `2eed25e`) quebraram 16 testes que não tinham sido revalidados. (a) `engine-orchestrator.ts`: a rotina que preenche 10 vagas de `VENDA_ESTADO` com os piores pontuadores podia varrer casos que **acabaram de virar `MATCH`/`MATCH_PARCIAL` no mesmo run** — corrigido excluindo `result_status IN ('MATCH','MATCH_PARCIAL')` da consulta de candidatos. (b) `repair-service.ts::createRepairCase`: migration 050 (índice único parcial `idx_repair_cases_active_imei`) não tinha guarda de aplicação — violação virava erro cru do SQLite; adicionado check prévio que lança `RepairError("DUPLICATE_ACTIVE_IMEI", ...)` amigável (já tratado por `handleRepairError` nas rotas). (c) Testes desatualizados ajustados: `tests/repair-domain.test.ts` assumia múltiplos casos ativos por IMEI (regra antiga, pré-050) — reescritos para refletir "1 caso ativo por IMEI, reabertura permitida após encerramento"; `tests/match-engine-integration.test.ts` e `tests/receipt-motor.test.ts` ganharam `seedFillerCases()` (10 casos já em `VENDA_ESTADO`) para não deixar o caso sob teste ser o único elegível e virar vaga de liquidação em bancos pequenos. Trabalho de UI (`repaginação`, dashboards, migration 053 `venda_estado_count`) já estava presente e correto — não precisou de mudança, só ficou destravado pelos testes.
